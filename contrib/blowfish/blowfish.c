...
void main( int argc, char *argv[] ) {
	...
	if( argc < 3 ) {
		fprintf( stdout, "%s <key> { <plaintext> <ciphertext> | <ciphertext> }\n", argv[0] );
		exit(0);
	}

	Blowfish_Init( &ctx, (unsigned char*)argv[1], strlen(argv[1]) );

	if( stat( argv[2], &info ) ) {
		err( -1, "stating %s", argv[2] );
	}
	...
	if( argc > 3 ) { // encrypt
		...
		memset( buf, 0, SIZE );
		// Read
		fread( buf, sizeof(char), info.st_size, fp );
		fclose( fp );
		// Encrypt
		n = SIZE;
		while( n > 0 ) {
			Blowfish_Encrypt( &ctx, ptr, ptr+1 );
			ptr += 2;
			n -= 8;
		}
		// Write
		...
		assert( info.st_size % 8 == 0 );

		buf = calloc( info.st_size, sizeof(char));
		fp = fopen( argv[2], "r" );
		fread( buf, sizeof(char), info.st_size, fp );
		fclose( fp );
		ptr = (unsigned int *)buf;
		n = info.st_size;
		while( n > 0 ) {
			Blowfish_Decrypt( &ctx, ptr, ptr+1 );
			ptr += 2;
			n -= 8;
		}
		fputs( buf, stdout );
